...
/**
 * Example usage of OpenHtmlToPDFUtils for converting HTML to PDF
 * This demonstrates the capabilities including Chinese character support and image embedding
 */
@Slf4j
public class OpenHtmlToPDFUtilsExample {
    ...
    private static void example1SimpleConversion() throws Exception {
        ...
        String html = "<html><body>" +
                "<h1>Hello World</h1>" +
                "<p>This is a simple PDF generated from HTML using OpenHTMLToPDF.</p>" +
                "</body></html>";
        
        byte[] pdfBytes = OpenHtmlToPDFUtils.htmlToPdf(html);
        
        Path outputPath = Paths.get("/tmp/openhtmltopdf_example1_simple.pdf");
    ...
    private static void example2ChineseCharacters() throws Exception {
        ...
        String html = "<!DOCTYPE html>" +
                ...
                "<body>" +
                "<h1>中文文檔範例</h1>" +
                "<p>這是一個支援中文的 PDF 文件，使用 OpenHTMLToPDF 庫生成。</p>" +
                "<p>繁體中文：台灣、香港、澳門</p>" +
                ...
                "<li>可以嵌入圖片</li>" +
                "<li>使用 OpenHTMLToPDF 庫進行渲染</li>" +
                "</ul>" +
                "</body>" +
                "</html>";
        
        Path outputPath = Paths.get("/tmp/openhtmltopdf_example2_chinese.pdf");
    ...
    private static void example3WithImages() throws Exception {
        ...
        byte[] pdfBytes = OpenHtmlToPDFUtils.htmlToPdf(html);
        Path outputPath = Paths.get("/tmp/openhtmltopdf_example3_images.pdf");
        Files.write(outputPath, pdfBytes);
    ...
    private static void example4ComplexHtml() throws Exception {
        ...
        String html = "<!DOCTYPE html>" +
                ...
                "<h1>員工資訊報表</h1>" +
                "<div class=\"info-box\">" +
                "<strong>報表日期：</strong> 2025年10月16日<br/>" +
                "<strong>生成工具：</strong> OpenHTMLToPDF" +
                "</div>" +
                "<table>" +
                "<tr>" +
                "<th>員工編號</th>" +
        ...
        byte[] pdfBytes = OpenHtmlToPDFUtils.htmlToPdf(html);
        Path outputPath = Paths.get("/tmp/openhtmltopdf_example4_complex.pdf");
        Files.write(outputPath, pdfBytes);
